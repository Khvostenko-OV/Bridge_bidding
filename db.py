...
from config import DB_NAME, DB_DSN
from models import Bid
from utils import decomp_seq, can_pass, can_contra, can_recontra, seq2list, list2seq
...
def get_system(name: str, db=DB_DSN) -> dict | None:
    """Get system info"""
...
def add_answer(system_name: str, seq: str="", opps=False, db=DB_DSN) -> bool:
    ...
    if not opps and seq:
        bid.seq += ".0"
    return upsert_bid(system_name, bid, db)

def del_thread(system_name: str, seq: str, db=DB_DSN) -> int:
    ...
    try:
        conn = psycopg2.connect(**db)
        cursor = conn.cursor()
        cursor.execute(f'DELETE FROM "{system_name}" WHERE seq LIKE %s;', (seq+"%",))
        del1 = cursor.rowcount
        cursor.execute(f'DELETE FROM "{system_name}" WHERE seq=%s AND bid=%s;', (prev_seq, bid))
